 ...
 *   |acceleration|
 *
 * KalmanFilter_inertial_t Height_KF;
 *
 ...
 *       | 0   0  35|
 *
 *     Kalman_Filter_Inertial_Init(&Height_KF, 3, 0, 3);
 *
 *     // 设置矩阵值
 ...
 * {
 *     // 循环更新
 *     Kalman_Filter_Inertial_Update(&Height_KF);
 *     vTaskDelay(ts);
 * }
 *
...
static void H_K_R_Adjustment(KalmanFilter_inertial_t *kf);
 ...
 * @param zSize 观测量维度
 */
void Kalman_Filter_Inertial_Init(KalmanFilter_inertial_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
...
void Kalman_Filter_Inertial_Measure(KalmanFilter_inertial_t *kf)
{
    // 矩阵H K R根据量测情况自动调整
    // matrix H K R automatic adjustment
    if (kf->UseAutomaticAdjustment != 0)
        H_K_R_Adjustment(kf);
    else
    {
...
void Kalman_Filter_Inertial_xhatMinusUpdate(KalmanFilter_inertial_t *kf)
{
    if (!kf->SkipEq1)
    {
        if (kf->uSize > 0)
...
void Kalman_Filter_Inertial_PminusUpdate(KalmanFilter_inertial_t *kf)
{
    if (!kf->SkipEq2)
    {
...
void Kalman_Filter_Inertial_SetK(KalmanFilter_inertial_t *kf)
{
    if (!kf->SkipEq3)
    {
...
void Kalman_Filter_Inertial_xhatUpdate(KalmanFilter_inertial_t *kf)
{
    if (!kf->SkipEq4)
    {
...
void Kalman_Filter_Inertial_P_Update(KalmanFilter_inertial_t *kf)
{
    if (!kf->SkipEq5)
    {
 ...
 * @return float* 返回滤波值
 */
float *Kalman_Filter_Inertial_Update(KalmanFilter_inertial_t *kf)
{
    // 0. 获取量测信息
    Kalman_Filter_Inertial_Measure(kf);
    if (kf->User_Func0_f != NULL)
        kf->User_Func0_f(kf);

    // 先验估计
    // 1. xhat'(k)= A・xhat(k-1) + B・u
    Kalman_Filter_Inertial_xhatMinusUpdate(kf);
    if (kf->User_Func1_f != NULL)
        kf->User_Func1_f(kf);

    // 预测更新
    // 2. P'(k) = A・P(k-1)・AT + Q
    Kalman_Filter_Inertial_PminusUpdate(kf);
    if (kf->User_Func2_f != NULL)
        kf->User_Func2_f(kf);

    if (kf->MeasurementValidNum != 0 || kf->UseAutomaticAdjustment == 0)
    {
        // 量测更新
        // 3. K(k) = P'(k)・HT / (H・P'(k)・HT + R)
        Kalman_Filter_Inertial_SetK(kf);
        ...
        Kalman_Filter_Inertial_xhatUpdate(kf);
        ...
        Kalman_Filter_Inertial_P_Update(kf);
    }
    else
    {
        // 无有效量测,仅预测
