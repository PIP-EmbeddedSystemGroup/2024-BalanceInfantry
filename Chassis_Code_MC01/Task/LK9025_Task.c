...
void LK9025_Tx_Task(void const *argument)
{
	...
	float target_rotate_speed = 2.1f;
	...
	while (1)
	{
		...
		if (Command.Chassis_Power_Switch == ON)
		{
			...
			{
                float rotateForce = (target_rotate_speed - Ins_Data.Gyro[1]) * 2.5;
				LK9025_Output_Normal(-Leg[LEFT].U[0] + rotateForce, Leg[RIGHT].U[0] + rotateForce);
			}
